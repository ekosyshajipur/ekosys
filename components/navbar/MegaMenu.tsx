...
export default function MegaMenu({ onClose, onMouseLeave }: MegaMenuProps) {
  return (
    <div
      className="mt-3 w-full bg-white rounded-3xl border border-slate-200/90 shadow-2xl overflow-hidden p-6 lg:p-8 animate-in fade-in slide-in-from-top-2 duration-200 pointer-events-auto"
        ...
        {DIVISIONS.map((division, index) => {
          ...
          return (
            <Link
              key={division.id}
              href={division.link}
              className="group p-4 rounded-2xl hover:bg-slate-50 transition-all duration-200 border border-transparent hover:border-slate-100"
              onClick={onClose}
              role="menuitem"
            >
